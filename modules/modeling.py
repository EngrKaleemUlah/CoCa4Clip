...
from modules.visualEncoder import ViTFeatureExtractor
            ...
            if contain_cross is False:
                for key, val in clip_state_dict.items():
                    if key == "positional_embedding":
                        state_dict["cross.embeddings.position_embeddings.weight"] = (
                            val.clone()
                        )
                        continue
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        modelname = "efficientvit_b3.r288_in1k"
        self.visionEncoder = ViTFeatureExtractor(model_name=modelname).to("cuda")
    ...
    def get_sequence_output(
        self, encodedSentence, input_ids, token_type_ids, attention_mask, shaped=False
    ):
        # print(f"shape of encodedSentence before unsqueeze: {encodedSentence.shape}")
        encodedSentence = encodedSentence.unsqueeze(1)
        # print(f"shape of encodedSentence after unsqueeze: {encodedSentence.shape}")
        if shaped is False:
            input_ids = input_ids.view(-1, input_ids.shape[-1])
            token_type_ids = token_type_ids.view(-1, token_type_ids.shape[-1])
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
        ...
        # sequence_hidden = self.clip.encode_text(input_ids.to("cuda")).float()
        # sequence_hidden = self.coca.forward_text(input_ids.to("cuda")).float()
        # sequence_hidden = sequence_hidden.view(bs_pair, -1, sequence_hidden.size(-1))
        # print(
        #     f"SequenceHidden shape: {sequence_hidden.shape}, encodedSentence shape: {encodedSentence.shape}"
        # )
        # input("Waiting....")
        # combined_embedding = (sequence_hidden + encodedSentence) / 2
        # combined_embedding = F.normalize(combined_embedding, dim=-1)
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        # visual_hidden = self.clip.encode_image(video, video_frame=video_frame).float() (original)
        visual_hidden = self.coca.forward_image(video, video_frame=video_frame).float()
        visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1))
        # visual_hidden_vit = self.visionEncoder(video, video_frame)
        # print(
        #     f"Visual Hidden shape:{visual_hidden.shape} visual_hidden_vit shape: {visual_hidden.shape}"
        # )
        # input("Waiting....")
        # visual_hidden_combined = (visual_hidden + visual_hidden_vit) / 2
        # visual_hidden_combined = F.normalize(visual_hidden_combined, dim=-1)
        return visual_hidden
        # return visual_hidden_combined
    ...
    def get_sequence_visual_output(
        ...
        sequence_output = self.get_sequence_output(
            encodedSentence, input_ids, token_type_ids, attention_mask, shaped=True
        )
...
#         self.clip = CoCaWrapper(model_name="coca_ViT-L-14")
