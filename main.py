...
if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
    ...
    print("[INFO] Running in non-distributed (single-GPU/CPU) mode.")
    distributed = False
...
def get_args(description="CLIP4Clip on Retrieval Task"):
    ...
    #     "--coca_model_name",
    #     type="str",
    #     default="openai/coca-base",
    #     help="model name for coca.",
    # )
    ...
    parser.add_argument(
        "--lr", type=float, default=2.5e-5, help="initial learning rate"
    )
...
def set_seed_logger(args):
    ...
    # Handle distributed/non-distributed modes
    if dist.is_available() and dist.is_initialized():
        ...
        rank = 0
        local_rank = 0
        args.local_rank = 0  # ensure it's consistent
...
def train_epoch(
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        encodedSentence, input_ids, input_mask, segment_ids, video, video_mask = batch
        loss = model(
            encodedSentence, input_ids, segment_ids, input_mask, video, video_mask
        )
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            encodedSentence, input_ids, input_mask, segment_ids, video, video_mask = (
                batch
            )
            ...
            if multi_sentence_:
                ...
                sequence_output = model.get_sequence_output(
                    encodedSentence, input_ids, segment_ids, input_mask
                )
...
def main():
    ...
    assert (
        DATALOADER_DICT[args.datatype]["test"] is not None
